...
void
mswm::handle_push(Fl_Window& window, hit_type const what) const
{
  struct block* blk;
  int left = window.x() - 1;
  int top = window.y() - 1;
  ...
  Fl::draw_frame(left, top, width, height,
                 window.Fl_Widget::skin_.disabled_fcolor,
                 window.Fl_Widget::skin_.normal_bcolor);

  do
  {

    WaitForSingleObject(_screen_stdin, INFINITE);
    ...
    if (rc && count)
    {
      ...
      pushed = record.Event.MouseEvent.dwButtonState;
    }
    ...
    int delta_x = (record.Event.MouseEvent.dwMousePosition.X - Fl::e_x_root);
    int delta_y = (record.Event.MouseEvent.dwMousePosition.Y - Fl::e_y_root);

    Fl::e_x_root = record.Event.MouseEvent.dwMousePosition.X;
    Fl::e_y_root = record.Event.MouseEvent.dwMousePosition.Y;

    if (MOUSE_MOVED & record.Event.MouseEvent.dwEventFlags)
    {
      int left1 = left;
      int top1 = top;
