...
unsigned char*    Fl::e_text = (unsigned char*)"";
int   Fl::e_length;
const unsigned char*  Fl::e_clipboard_type = (unsigned char*)"";
...
*/
int
Fl::event_inside(int xx, int yy, int ww, int hh) /*const*/
{
  int mx = e_x - xx;
  int my = e_y - yy;
...
*/
int
Fl::event_inside(const Fl_Widget* o) /*const*/
{
  int mx = e_x - o->x();
  int my = e_y - o->y();
...
void
Fl::add_timeout(double time, Fl_Timeout_Handler cb, void* argp)
{
  Fl::screen_driver()->add_timeout(time, cb, argp);
}

void
Fl::repeat_timeout(double time, Fl_Timeout_Handler cb, void* argp)
{
 ...
 */
int
Fl::has_timeout(Fl_Timeout_Handler cb, void* argp)
{
 ...
 */
void
Fl::remove_timeout(Fl_Timeout_Handler cb, void* argp)
{
...
*/
void
Fl::add_check(Fl_Timeout_Handler cb, void* argp)
{
  Check* t = free_check;
...
*/
void
Fl::remove_check(Fl_Timeout_Handler cb, void* argp)
{
...
*/
int
Fl::has_check(Fl_Timeout_Handler cb, void* argp)
{
  for (Check** p = &first_check; *p;)
  {
    Check* t = *p;
...
void
Fl::add_clipboard_notify(Fl_Clipboard_Notify_Handler h, void* data)
{
...
bool
fl_clipboard_notify_empty(void)
{
  return clip_notify_list == NULL;
}

void
fl_trigger_clipboard_notify(int source)
{
...
*/
Fl_Window*
Fl::next_window(const Fl_Window* window)
{
 ...
 */
void
Fl::first_window(Fl_Window* window)
{
...
// escalates damage bits if a window overlaps another
// returns number of damaged (dirty) windows
static int
flush_propogate(Fl_X* i)
{
  int dirty = 0;
  ...
  do
  {
    ...
    dirty += flush_propogate(i->next);

    Fl_Window* wi = i->w;

    if (wi->damage())
    {
      if (0 == (Fl_Widget::FL_DAMAGE_ALL & wi->damage()))
      {
        for (Fl_X* n = i->next; n; n = n->next)
        {
          ...
          if (nw->damage())
          {
            ...
            if (overlap)
            {
              wi->damage(Fl_Widget::FL_DAMAGE_ALL);
              break;
            }
          }
        }
...
*/
void
Fl::remove_handler(Fl_Event_Handler ha)
{
  handler_link* l, *p;
...
static bool
send_handlers(Fl_Event const e)
{
  for (const handler_link* hl = handlers; hl; hl = hl->next)
    if (hl->handle(e)) return true;
...
*/
void
Fl::add_system_handler(Fl_System_Handler ha, void* data)
{
...
*/
void
Fl::remove_system_handler(Fl_System_Handler ha)
{
...
int
fl_send_system_handlers(void* e)
{
...
static int
send_event(Fl_Event event, Fl_Widget* to, Fl_Window* window)
{
  int dx, dy;
  Fl_Event old_event = Fl::e_number;
 ...
 */
bool
Fl::handle(Fl_Event e, Fl_Window* window)
{
  if (e_dispatch)
  {
 ...
 */
bool
Fl::handle_(Fl_Event e, Fl_Window* window)
{
  e_number = e;
  ...
  switch (e)
  {
    ...
    case FL_PUSH:
      ...
      else if (modal() && wi != modal()) return false;

      pushed_ = wi;

//    Fl_Tooltip::current(wi);
      if (send_event(e, wi, window)) return true;
...
//    Fl_Tooltip::enter((Fl_Widget*)0);
    ...
    case FL_SHORTCUT:
      ...
      {
        if (send_event(FL_SHORTCUT, first_window(), first_window())) return true;
      }

      for (; wi; wi = wi->parent())
      {
        if (send_event(FL_SHORTCUT, wi, wi->window())) return true;
      }

      // try using add_handle() functions:
      if (send_handlers(FL_SHORTCUT)) return true;
...
*/
void
Fl::selection_owner(Fl_Widget* owner)
{
...
*/
void
Fl::selection(Fl_Widget& owner, const unsigned char* text, int len)
{
  selection_owner_ = &owner;
  Fl::copy(text, len, 0);
}
...
//    if (box() == FL_BOX_NONE)
    {
      ...
      int X = x() > 0 ? x() - 1 : 0;
      int Y = y() > 0 ? y() - 1 : 0;
      window()->damage(Fl_Widget::FL_DAMAGE_ALL, X, Y, w() + 2, h() + 2);
    }

    if (align() && !(align() & Fl_Label::FL_ALIGN_INSIDE) && window()->shown())
    {
      ...
      switch (align() & 0x0f)
      {
        case Fl_Label::FL_ALIGN_TOP_LEFT:
          window()->damage(Fl_Widget::FL_DAMAGE_EXPOSE, x(), y() - H, W, H);
          break;

        case Fl_Label::FL_ALIGN_TOP:
          window()->damage(Fl_Widget::FL_DAMAGE_EXPOSE, x() + (w() - W) / 2, y() - H, W,
                           H);
          break;

        case Fl_Label::FL_ALIGN_TOP_RIGHT:
          window()->damage(Fl_Widget::FL_DAMAGE_EXPOSE, x() + w() - W, y() - H, W, H);
          break;

        case Fl_Label::FL_ALIGN_LEFT_TOP:
          window()->damage(Fl_Widget::FL_DAMAGE_EXPOSE, x() - W, y(), W, H);
          break;

        case Fl_Label::FL_ALIGN_RIGHT_TOP:
          window()->damage(Fl_Widget::FL_DAMAGE_EXPOSE, x() + w(), y(), W, H);
          break;

        case Fl_Label::FL_ALIGN_LEFT:
          window()->damage(Fl_Widget::FL_DAMAGE_EXPOSE, x() - W, y() + (h() - H) / 2, W,
                           H);
          break;

        case Fl_Label::FL_ALIGN_RIGHT:
          window()->damage(Fl_Widget::FL_DAMAGE_EXPOSE, x() + w(), y() + (h() - H) / 2, W,
                           H);
          break;

        case Fl_Label::FL_ALIGN_LEFT_BOTTOM:
          window()->damage(Fl_Widget::FL_DAMAGE_EXPOSE, x() - W, y() + h() - H, W, H);
          break;

        case Fl_Label::FL_ALIGN_RIGHT_BOTTOM:
          window()->damage(Fl_Widget::FL_DAMAGE_EXPOSE, x() + w(), y() + h() - H, W, H);
          break;

        case Fl_Label::FL_ALIGN_BOTTOM_LEFT:
          window()->damage(Fl_Widget::FL_DAMAGE_EXPOSE, x(), y() + h(), W, H);
          break;

        case Fl_Label::FL_ALIGN_BOTTOM:
          window()->damage(Fl_Widget::FL_DAMAGE_EXPOSE, x() + (w() - W) / 2, y() + h(), W,
                           H);
          break;

        case Fl_Label::FL_ALIGN_BOTTOM_RIGHT:
          window()->damage(Fl_Widget::FL_DAMAGE_EXPOSE, x() + w() - W, y() + h(), W, H);
          break;

        default:
          window()->damage(Fl_Widget::FL_DAMAGE_ALL);
          break;
      }
    }
...
void
Fl_Widget::damage(unsigned char fl)
{
  if (type() < FL_WINDOW)
  {
  ...
  else
  {
    ...
    damage_ = static_cast<Fl_Damage>((static_cast<unsigned char>(damage_) | fl));
    Fl::damage(Fl_Widget::FL_DAMAGE_CHILD);
  }
}

void
Fl_Widget::damage(unsigned char const fl, int const X, int const Y,
                  unsigned int const W, unsigned int const  H)
{
  Fl_Widget* wi = this;
  unsigned char l_damage = fl;
  int x = X;
  int y = Y;
  unsigned int w = W;
  unsigned int h = H;
  ...
  {
    wi->damage_ = static_cast<Fl_Damage>((static_cast<unsigned char>
                                          (wi->damage_) | fl));
    wi = wi->parent();

    if (!wi) return;

    l_damage = Fl_Widget::FL_DAMAGE_CHILD;
  }
  ...
  {
    wi->damage_ = static_cast<Fl_Damage>(l_damage);
  }
...
*/
void
Fl::watch_widget_pointer(Fl_Widget*& w)
{
  Fl_Widget** wp = &w;
  int i;
  ...
  {
    max_widget_watch += 8;
    widget_watch = (Fl_Widget***)realloc(widget_watch,
                                         sizeof(Fl_Widget**)*max_widget_watch);
  }
...
*/
void
Fl::release_widget_pointer(Fl_Widget*& w)
{
  Fl_Widget** wp = &w;
  int i, j = 0;
...
*/
void
Fl::clear_widget_pointer(Fl_Widget const* w)
{
  if (w == 0L) return;
 ...
 */
void
Fl::option(Fl_Option opt, bool val)
{
  if (opt < 0 || opt >= OPTION_LAST)
    return;
...
*/
void
Fl::insertion_point_location(int x, int y, int height)
{
...
const char*
fl_filename_name(const unsigned char* name)
{
  return Fl::system_driver()->filename_name((char*)name);
}

void
Fl::copy(const unsigned char* stuff, int len, int clipboard, const char* type)
{
  Fl::system_driver()->copy((char*)stuff, len, clipboard, type);
}

int
Fl::clipboard_contains(const char* type)
{
 ...
 */
void
fl_open_callback(void (*cb)(const char*))
{
