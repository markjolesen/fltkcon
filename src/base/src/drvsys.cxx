...
void
Fl_System_Driver::warning(const char* format, ...)
{
  va_list args;
...
void
Fl_System_Driver::warning(const char* format, va_list args)
{
...
void
Fl_System_Driver::error(const char* format, ...)
{
  va_list args;
  va_start(args, format);
...
void
Fl_System_Driver::error(const char* format, va_list args)
{
  vfprintf(stderr, format, args);
  fputc('\n', stderr);
  fflush(stderr);
...
void
Fl_System_Driver::fatal(const char* format, ...)
{
  va_list args;
  va_start(args, format);
...
void
Fl_System_Driver::fatal(const char* format, va_list args)
{
  vfprintf(stderr, format, args);
  fputc('\n', stderr);
  fflush(stderr);
...
int
Fl_System_Driver::XParseGeometry(const char* string, int* x, int* y,
...
unsigned
Fl_System_Driver::utf8towc(const char* src, unsigned srclen, wchar_t* dst,
                           unsigned dstlen)
{
  const char* p = src;
...
unsigned
Fl_System_Driver::utf8fromwc(char* dst, unsigned dstlen, const wchar_t* src,
                             unsigned srclen)
{
  unsigned i = 0;
  unsigned count = 0;
...
unsigned
Fl_System_Driver::utf8to_mb(const char* src, unsigned srclen, char* dst,
                            unsigned dstlen)
{
  wchar_t lbuf[1024];
  wchar_t* buf = lbuf;
...
unsigned
Fl_System_Driver::utf8from_mb(char* dst, unsigned dstlen, const char* src,
                              unsigned srclen)
{
  wchar_t lbuf[1024];
  wchar_t* buf = lbuf;
...
int
Fl_System_Driver::clocale_printf(FILE* output, const char* format,
                                 va_list args)
{
...
int
Fl_System_Driver::filename_expand(char* to, int tolen, const char* from)
{
  ...
  for (char* a = temp; a < end; ) // for each slash component
  {
    ...
    for (e = a; e < end && *e != '/'; e++) {/*empty*/} // find next slash
...
int
Fl_System_Driver::file_browser_load_directory(const char* directory,
                                              char* filename,
...
int
Fl_System_Driver::file_type(const char* filename)
{
  return 0;
}

FILE*
Fl_System_Driver::fopen(const char* f, const char* mode)
{
  return ::fopen(f, mode);
}

void
Fl_System_Driver::open_callback(void (*)(const char*))
{
}

// Get elapsed time since Jan 1st, 1970.
void
Fl_System_Driver::gettime(time_t* sec, int* usec)
{
  *sec =  time(NULL);
  *usec = 0;
}
