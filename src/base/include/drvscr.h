...
#include "texted.h"
#include "fl.h" // for Fl_Timeout_Handler
...
class Fl_Screen_Driver
{
  ...
  public:
    ...
    w()
    {
      return 800;  // default, override in driver!
    }
    virtual int
    h()
    {
      return 600;  // default, override in driver!
    }
    ...
    virtual int parse_color(const char* p, unsigned char& r, unsigned char& g,
                            unsigned char& b);
    virtual void
    get_system_colors() { }
    ...
    reset_marked_text() {}
    virtual void
    insertion_point_location(int x, int y, int height) {}
    ...
    virtual bool input_widget_handle_key(int key, unsigned mods, unsigned shift,
                                         Fl_Input* input);
